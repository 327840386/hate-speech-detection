...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            model.train()
            total_loss = torch.tensor(0.0, device=local_rank if (train_config.enable_fsdp or train_config.enable_ddp) else 'cuda:0')
            total_acc = torch.tensor(0.0, device=local_rank if (train_config.enable_fsdp or train_config.enable_ddp) else 'cuda:0')
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (epoch * total_length + step + 1) % train_config.validation_interval == 0 and train_config.run_validation:
                    ...
                    if eval_epoch_acc > best_val_acc:
                        ...
                        if train_config.enable_fsdp or train_config.enable_ddp:
                            if rank==0:
                                logger.info(f"New best accuracy on epoch {epoch+1}: {best_val_acc:.4f}")
                        else:
                            logger.info(f"New best accuracy on epoch {epoch+1}: {best_val_acc:.4f}")
                        ...
                        if train_config.enable_fsdp or train_config.enable_ddp:
                            if rank==0:
                                wandb.log({
                                    ...
                                    "valid/val_accuracy": eval_epoch_acc,
        ...
        train_perplexity = torch.exp(train_epoch_loss).item()
...
def evaluation(model, train_config, eval_dataloader, local_rank, tokenizer):
    """Evaluation with specific handling for hate speech detection"""
    if train_config.enable_fsdp or train_config.enable_ddp:
        world_size = int(os.environ["WORLD_SIZE"])
    ...
    with MemoryTrace() as memtrace:
        ...
        pbar = tqdm(colour="green", desc="Evaluating", total=total_length)
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                with autocast():
                    ...
                    if hasattr(batch, 'targets'):
                        ...
                        predictions = model.clean_output_text(
                            tokenizer.decode(outputs[0], skip_special_tokens=True)
                        )
                        acc = float(predictions == targets)
                    else:
                        acc = rest[0] if rest else 0.0
            ...
            pbar.set_description(
                f"Step: {step+1}/{total_length}, "
                f"Loss: {eval_loss/(step+1):.4f}, "
                f"Acc: {eval_acc/(step+1):.4f}"
            )
    ...
    eval_ppl = torch.exp(eval_epoch_loss_tensor).item()
