...
def main(kwargs: DictConfig):
    ...
    if not torch.cuda.is_available():
        ...
        # 获取local_rank，如果未指定则默认为0
        local_rank = int(os.environ.get("LOCAL_RANK", "0"))
        
        # 确保local_rank在有效范围内
        if local_rank >= num_gpus:
            logger.warning(f"Local rank {local_rank} exceeds number of available GPUs {num_gpus}. Setting to 0.")
            local_rank = 0
        ...
        try:
            ...
            logger.info(f"Using GPU {local_rank} of {num_gpus} GPUs")
        except RuntimeError as e:
            logger.warning(f"Failed to set CUDA device: {e}. Falling back to CPU.")
            device = torch.device("cpu")
            local_rank = -1
