...
class AudioAugmentation:
    ...
    def time_stretch(self, audio, sr=16000):
...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.data_list = []
        if split == "train":
            with open(dataset_config.train_data_path, encoding='utf-8-sig') as fin:
                for line in fin:
        ...
        self.use_augmentation = (
            split == 'train' and 
            getattr(dataset_config, 'use_data_augmentation', False)
        )
        if self.use_augmentation:
            self.augmentation = AudioAugmentation(p=0.5)
            print(f"Data augmentation enabled for {split} split")
    ...
    def __getitem__(self, index):
        ...
        target = data_dict.get("hate_speech", "NO") #default NO
        task = data_dict.get("prompt", "HATE-SPEECH-DETECTION")
        key = data_dict.get("key", None)

        audio_raw = whisper.load_audio(audio_path)

        # 应用数据增强
        if hasattr(self, 'use_augmentation') and self.use_augmentation and not self.inference_mode:
            try:
                audio_raw = self.augmentation.apply(audio_raw)
            except Exception as e:
                print(f"Warning: Failed to apply augmentation to {audio_path} - {e}")

        if self.input_type == "raw":
            # 确保数据类型为 float32
            audio_raw = torch.from_numpy(audio_raw).float()  # 明确指定为 float
            if self.normalize:
            ...
            # 确保数据类型为 float32
            audio_mel = whisper.log_mel_spectrogram(audio_raw, n_mels=self.mel_size).float().permute(1, 0)
        ...
        if prompt is None:
            ...
            prompt = "Analyze the following audio input and determine if it contains any instances of hate speech based on abusive, threatening, or discriminatory language related to race, ethnicity, religion, gender, sexual orientation, or other protected characteristics. Respond only with 'Yes' or 'No'. "
